  ...
  const handleFaceAuthAction = async (action: 'login' | 'register') => {
    ...
    try {
        if (action === 'register') {
            ...
            const result = await registerWithFaceID(registerData);
            if (result.success) {
                toast({ title: 'Cadastro bem-sucedido!', description: result.message || 'Seu rosto e dados foram registrados.' });
                if (result.redirectUrl) {
                  router.push(result.redirectUrl);
                } else {
                  router.push('/auth/face?tab=signin');
                }
            } else {
                 toast({ variant: 'destructive', title: 'Falha no Cadastro', description: result.message || 'Não foi possível completar seu registro.' });
            }
        } else { // 'login'
            const loginData: LoginFaceIDInput = {
              image: imageBase64,
              email: loginEmail || undefined
            };
            
            const result = await loginWithFaceID(loginData);
            if (result.success) {
                toast({ title: 'Login bem-sucedido!', description: result.message || 'Autenticação realizada com sucesso!' });
                
                // Usar o contexto de autenticação
                const userTypeResult = result.isVip ? 'vip' : 'member';
                login(userTypeResult, result.userEmail);
                
                // Verificar se é VIP para redirecionar adequadamente
                if (result.isVip) {
                  router.push('/exclusivo');
                    ...
                    <TabsTrigger value="signup" className="data-[state=active]:bg-primary/90 data-[state=active]:text-primary-foreground data-[state=active]:shadow-neon-red-light">Cadastrar</TabsTrigger>
                        ...
                        <TabsContent value="signup">
                            <div className="space-y-4 pt-4">
                                ...
                                <Button onClick={() => handleFaceAuthAction('register')} disabled={!hasCameraPermission || !isVideoReady || isVerifying || !name || !loginEmail} className="w-full justify-center h-12 text-base bg-primary/90 hover:bg-primary text-primary-foreground shadow-neon-red-light hover:shadow-neon-red-strong">
                                <Fingerprint className="w-5 h-5 mr-2" />
