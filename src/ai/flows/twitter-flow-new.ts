...
function processRapidApiV2Data(data: any, mediaType: 'photos' | 'videos' | 'all', username: string): TwitterMediaOutput {
    ...
    for (const entry of entries) {
        ...
        if (filteredMedia.length > 0 || mediaType === 'all') {
            tweets.push({
                id: tweet.rest_id,
                ...
                isRetweet: tweet.legacy?.retweeted || false
            });
        }

        // Fotos repostadas (retweets)
        const retweeted = tweet?.legacy?.retweeted_status_result?.result;
        if (retweeted) {
            const retweetMedia = retweeted?.legacy?.entities?.media || retweeted?.legacy?.extended_entities?.media || [];
            const filteredRetweetMedia = retweetMedia.filter((m: any) => {
                if (mediaType === 'photos') return m.type === 'photo';
                if (mediaType === 'videos') return m.type === 'video' || m.type === 'animated_gif';
                return true;
            });

            if (filteredRetweetMedia.length > 0 || mediaType === 'all') {
                tweets.push({
                    id: retweeted.rest_id,
                    text: retweeted.legacy?.full_text || '',
                    media: filteredRetweetMedia.map((m: any) => ({
                        media_key: m.media_key || m.id_str || '',
                        type: m.type,
                        url: m.media_url_https || '',
                        preview_image_url: m.media_url_https || ''
                    })),
                    created_at: retweeted.legacy?.created_at || new Date().toISOString(),
                    username,
                    profile_image_url: retweeted.core?.user_results?.result?.legacy?.profile_image_url_https || '',
                    isRetweet: true
                });
            }
        }
    ...
    const processedTweets = tweets
        .map((tweet: any) => {
            ...
            return {
                ...
                created_at: tweet.created_at || new Date().toISOString(),
                username,
                isRetweet: tweet.full_text?.startsWith('RT @') || false
            };
        })
        .filter(Boolean);
