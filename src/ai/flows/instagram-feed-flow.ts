...
const fetchInstagramProfileMediaFlow = ai.defineFlow(
  ...
  async () => {
    ...
    try {
      ...
      console.log(`Instagram Feed Token disponível: ${!!accessToken}`);
      if (accessToken) console.log(`Instagram Feed Token inicia com: ${accessToken.substring(0, 10)}...`);
    } catch (error: any) {
      console.error('Erro ao acessar token do Instagram Feed:', error);
      return { media: [], error: `Não foi possível carregar as fotos do Instagram. Motivo: Erro ao acessar token: ${error.message}` };
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        try {
            ...
            console.error("Resposta não-JSON da API do Instagram (Feed Flow):", errorText);
        }
        return { media: [], error: errorMessage };
      }
      ...
      return result;
        ...
        if (errorMessage.includes('decrypt') || errorMessage.includes('Failed to decrypt')) {
          ...
          console.error('Erro de descriptografia detectado:', error);
        }
