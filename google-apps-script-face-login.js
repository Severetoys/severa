  ...
  try {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      if (storedImage && storedImage.indexOf(imageToCompare) !== -1) {
        ...
        console.log(`[Face ID Login] Usuário encontrado: ${userName} (${userEmail}), VIP: ${hasPayment}`);
    ...
    console.log('[Face ID Login] Nenhum usuário correspondente encontrado');
    return {
      success: false,
    ...
    console.error('[Face ID Login] Erro na verificação:', error.message);
    return {
      success: false,
...
function doPost(e) {
  ...
  try {
    ...
    if (e.parameter && e.parameter.page === 'login') {
      console.log('[Apps Script] Processando login via Face ID...');
